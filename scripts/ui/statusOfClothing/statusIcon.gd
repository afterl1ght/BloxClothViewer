...
func bubble_pressed():
	...
	if $bubble.modulate.a > 0:
		...
		$bubble/Tween.interpolate_property($bubble, "modulate", $bubble.modulate, Color($bubble.modulate.r, $bubble.modulate.g, $bubble.modulate.b, 0), 1, Tween.TRANS_CUBIC, Tween.EASE_IN)
		$bubble/Tween.start()
...
func _pressed():
	if bubbleremainingTime == 0 and !bubbleVisible:
		...
		bubbleVisible = true
		$bubble/Tween.stop_all()
		$bubble/Tween.interpolate_property($bubble, "modulate", $bubble.modulate, Color($bubble.modulate.r, $bubble.modulate.g, $bubble.modulate.b, 1), 1, Tween.TRANS_CUBIC, Tween.EASE_OUT)
		$bubble/Tween.start()
